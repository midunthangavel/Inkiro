...
export default function LoginScreen({ onLogin }) {
  ...
  async function verifyOtp() {
    ...
    setLoading(true);
    try {
      const { data } = await api.post('/auth/verify-otp', { phone, code: otp, role: 'customer' });
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1 bg-paper">
        <View className="flex-1 px-6 pt-24" style={{ gap: 24 }}>
          <View>
            ...
            <Text className="text-ink-soft mt-3 leading-5">
              Groceries, delivered from your{'\n'}neighbourhood kadai.
            </Text>
            <Tamil size={13}>உங்கள் பக்கத்து கடையிலிருந்து பலசரக்கு</Tamil>
          </View>
          ...
          <View className="mt-auto mb-8">
            <Text className="text-center text-ink-muted text-xs leading-5">
              By continuing you agree to our{'\n'}
              <Text className="text-ink underline">terms</Text> and <Text className="text-ink underline">privacy</Text>
            </Text>
          </View>
        </View>
