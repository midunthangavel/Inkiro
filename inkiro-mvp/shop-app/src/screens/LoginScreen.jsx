...
export default function LoginScreen({ onLogin }) {
  ...
  async function verifyOtp() {
    ...
    setLoading(true);
    try {
      const { data } = await api.post('/auth/verify-otp', { phone, code: otp, role: 'shop' });
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1 bg-paper">
        <View className="flex-1 px-6 pt-24" style={{ gap: 24 }}>
          <View>
            ...
            <Text className="text-ink-muted text-[11px] font-semi tracking-widest uppercase mt-1">Shop</Text>
            <Text className="text-ink-soft mt-3 leading-5">Accept orders. Pack &amp; hand off.</Text>
            <Tamil size={13}>ஆர்டர்களை ஏற்று, பொதிந்து ரன்னருக்கு தரவும்</Tamil>
          </View>
